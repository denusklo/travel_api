...
class ToursListTest extends TestCase
{
    ...
    public function teest_tours_list_sorts_by_price_correctly(): void
    {
        $travel = Travel::factory()->create();
        ...
        $response = $this->get('/ap/v1/travels/'.$travel->slug.'/tours?sortBy=price&sortOrder=asc');

        $respnse->assertStatus(200);
        $respnse->assertJsonPath('data.0.id', $cheapEarlierTour->id);
        $respnse->assertJsonPath('data.1.id', $cheapLaterTour->id);
        $respnse->assertJsonPath('data.2.id', $expensiveTour->id);
    }
